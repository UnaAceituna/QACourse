...
Идентификация - "назови себя". Процедура, в результате которой для субъекта идентификации выявляется его идентификатор. То есть сначала система запрашивает логин.
Аутентификация - "докажи что ты есть ты". Проверка подлинности, проверка пароля.
Двухфакторная аутентификация - проверка с помощью смс, например.
Авторизация - "получи права на запрашиваемую страницу". Предоставление определённому лицу прав на выполнение каких-либо действий.

Авторизация без идентификации возможна - например публичные документы, для которыйх не нужно называть себя, но система всё равно распознаёт пользователя.
...
16) Уровни OSI - Open system intercorrection
В 1978 был разработан тандарт архитектуры OSI. Это открытая система сетевого уровняб модель описывает правила и процедуры передачи данных в различных сетевых средах при организации сеанса связи. Основные элементы - уровни, прикладные процессы и физические средства соединения.
Она была придумана для единого представления данных в сетях с неоднородными устройствами.
Уровни OSI
...
3. Сетеой уровень (network layer). Маршрутизация от одной сети к другой, определение кратчайшего пути, логическая адресация.
Маршрутизация - процесс определения лучшего пути, по которому пакет может быть доставлен получателю. Поювляются IP-адреса, утройства - маршутизаторы, или роутеры.Отправляются DNS-запросы на этом уровне, данные с прошлого уровня формируютсяв пакет.
4. Транспортный уровень (transport layer). Собирает информацию в информационный поток между двумя передающими системами, единица передачи информации - сегмент. Адресация происходит по портам, порт является идентификатором конкретного приложения.
Есть два вида доставки:
-Надёжная. Протокол TCP - запрашивает повторно данные в случае ошибки, проверяет целостность и доставку пакета. (HTTP, HTTPS, FTP, POP3, RDP)
-Ненадёжная. Протокол UDP. 
5. Сеансовый уровень (session layer). Управляет соединениями (сессиями). Например, соединение с веб-сервером. Методы передачи информации:
-Simplex
-Half-Duplex
-Full-Duplex
